import { Achievement } from '@/types/achievement'

export const achievement: Achievement[] = [
  {
    src: '/images/achievements/framer-logo.svg',
    srcDark: '/images/achievements/white-framer-logo.svg',
    title: 'Framer Awards',
    desc: 'Celebrated for cutting-edge interaction design and seamless user experiences.',
    year: 2024,
  },
  {
    src: '/images/achievements/dribble-logo.svg',
    srcDark: '/images/achievements/dribble-logo.svg',
    title: 'Dribbble Awards',
    desc: 'Recognized for creative excellence and innovative design solutions',
    year: 2023,
  },
  {
    src: '/images/achievements/awwards-logo.svg',
    srcDark: '/images/achievements/white-awwwards-logo.svg',
    title: 'awwwards Awards',
    desc: 'Honored with the Best Website Design for creativity, usability, and innovation.',
    year: 2022,
  },
]
